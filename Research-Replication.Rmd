...
tests_by_zcta <- read_csv(
  "https://raw.githubusercontent.com/nychealth/coronavirus-data/097cbd70aa00eb635b17b177bc4546b2fce21895/tests-by-zcta.csv", show_col_types = F)
```

## Variability in socioeconomic status among different zip codes in NYC

We replicate heat maps from the original article that shows how socioeconomic factors vary among Zip codes in NYC.

The resulting plot looks identical to the one plotted in article. 
...
We get R-squared value as same as reported in the original paper, which is 3%, with the model against proportion of elderly people. It appears that very little of variabilty in the response varialbe is due to people's choice to use bus for commute.

## Multivariable Regression model for COVID-19 positive cases

We build a multivariable regression model for COVID-19 positive cases where we take four out of six socioeconomic factors as our features. As done in the original article, we add variables in decreasing order of their R-squared value. We add proportion  living  in  households  with  four  or  more  indi-viduals, proportion of adults who were uninsured, proportion iden-tifying as white, and median household income. Further addition of variables results in marginal changes.
...
Next, we plot the second plot from the article. The plot contains violin plolt for each day in March and April of 2020. The violins represent distribution of mobility on that day. Mobility is definied as the movement of people to point of interests (POI). We take median of average visits per day (to POIs) from pre-pandemic as our baseline. The we calcualte change in mobility in a day relative to the baseline. Here, we could access the data for only February to model our baseline, however the article uses data from September to February. This could lead to changes in our plot, however it should produce similar trend.

The data for baselines and post-pandemic mobility are saved in the file *mobility.Rdata*. 
...
The original article did right calculations mostly, however its claim of change in mobility did not affect COVID-19 positive cases is misleading. The features studied seem to be less predictive because we obtain a model with low R-squared value. Doing more research on what factors might affect the COVID-19 positive cases and integrating them in the model could increase the predicitability or explain more variability in the response variable. Given the original research was conducted early in the pandemic, there might not have been sufficient information and data to conduct this research. And if we were to gather recent data we will need to consider other factors such as proportion of vaccinated population, which was absent during early months of the pandemic. We believe this factor alone would bring significant change in the model. We are also curious to know how this model would perform with more recent data on COVID-19 positive cases. It will also be interesting to understand variability on COVID-19 cases due to other means of transportation, for example majority of people in NYC commute by subway trains.
